...
usage() {
  cat <<'USAGE'
usage:
  ./scripts/ddb-cloud-control.sh [-Config path] [-Action validate|status|start|stop|restart]
USAGE
}
...
machine_name = str(cfg.get("machine_name", "")).strip() or "cloud-control"
data_root = resolve_path(cfg.get("data_root", ""), install_root, config_dir) or os.path.join(install_root, "data")
log_dir = resolve_path(cfg.get("log_dir", ""), install_root, config_dir) or os.path.join(install_root, "logs", "control-plane")
...
etcd_runner = str(etcd.get("runner", "native")).strip() or "native"
if etcd_runner not in {"docker", "native"}:
    raise SystemExit("etcd.runner must be docker or native")
etcd_port = int(etcd.get("port", 2379))
etcd_data_dir = resolve_path(etcd.get("data_dir", ""), install_root, config_dir) or os.path.join(install_root, "data", "etcd")
etcd_health_url = str(etcd.get("health_url", "")).strip() or f"http://{local_ip}:{etcd_port}/health"
etcd_binary = resolve_path(etcd.get("binary_path", ""), install_root, config_dir) or os.path.join(install_root, "bin", "etcd")
...
api_raft_dir = resolve_path(api.get("raft_dir", ""), install_root, config_dir) or os.path.join(data_root, api_node_id, "raft")
api_db_path = resolve_path(api.get("db_path", ""), install_root, config_dir) or os.path.join(data_root, api_node_id, "apiserver.db")
...
validate() {
  ...
  if [[ "$ETCD_RUNNER" == "docker" ]]; then
    ...
    docker_daemon_available || fail "docker is installed but daemon is not available"
  else
