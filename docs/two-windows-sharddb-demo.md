# Two-Windows ShardDB Demo Guide

这份文档记录了如何在两台 Windows 物理机上拉起一个 `3 group / 9 node` 的 `ShardDB` 集群。

目标是：

- `WIN_A` 运行 `etcd`、`apiserver`、`g1-n1`、`g2-n1`、`g3-n1`
- `WIN_B` 运行 `g1-n2`、`g1-n3`、`g2-n2`、`g2-n3`、`g3-n2`、`g3-n3`
...
- `etcd` 和 `apiserver` 单实例放在 `WIN_A`，用 Docker 管理更方便
- 双物理机场景下，`shard` 节点直接使用宿主机真实 IP 跑原生进程最稳
...
这套布局不是最均匀的，但足够完成双物理机验证、建表、写入、查询、`rebalance`、节点故障与恢复演示。

## 2. 准备阶段

### 2.1 安装 Go

两台 Windows 机器都需要安装 Go。
...
如果 `WIN_A` 无法安装或运行 Docker Desktop，也可以继续做这套双机实验：
...
- `6.2 原生进程方式启动 etcd`
- `9.1 Docker 方式启动 apiserver`
- `9.2 原生进程方式启动 apiserver`
...
```powershell
cd $env:DDB_ROOT
go run .\cmd\netcheck --peer=$env:WIN_B_IP --ports=21180,21280,21181,21281,21182,21282,22180,22280,22181,22281,22182,22282
```
...
.\bin\ddb-server.exe `
  ...
  --node-id=g1-n3 `
  --group-id=g1 `
  --http-addr=$env:WIN_B_IP:21280 `
  --raft-addr=$env:WIN_B_IP:22280 `
...
.\bin\ddb-server.exe `
  ...
  --node-id=g2-n3 `
  --group-id=g2 `
  --http-addr=$env:WIN_B_IP:21281 `
  --raft-addr=$env:WIN_B_IP:22281 `
...
.\bin\ddb-server.exe `
  ...
  --node-id=g3-n3 `
  --group-id=g3 `
  --http-addr=$env:WIN_B_IP:21282 `
  --raft-addr=$env:WIN_B_IP:22282 `
...
## 9. 最后在 WIN_A 启动 apiserver

### 9.1 方式 A：使用 Docker Desktop 启动 apiserver
...
### 13.2 看 apiserver 容器日志

```powershell
docker logs -f api-1
```

### 13.3 看 Windows shard 进程
...
推荐先停 `WIN_B` 上的 follower，例如：

- `g1-n2`
- `g2-n2`
- `g3-n2`

直接在对应 PowerShell 窗口 `Ctrl+C`。
